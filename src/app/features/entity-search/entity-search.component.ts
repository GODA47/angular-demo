...
@Component({
    ...
    template: `
        <main class="content-area relative w-full h-full">
    @if(showSearchHistoryPopup){
        <div class="absolute inset-0 bg-black/20 z-40" (click)="closePopup()">

        </div>
    }
    <div class="ml-0 flex-1 overflow-y-auto flex h-full items-start p-6">
        ...
        >
            <div class="flex items-center border-b-2 border-b-purple-800 mb-4 pb-3">
                ...
                <div class="relative ml-auto mr-0 flex items-center justify-center w-fit h-10 cursor-pointer"
                        (click)="openSearchHistoryPopup()">
                    <span class="text-purple-800 mr-1 underline font-semibold">Search History</span>
                    <span class="text-yellow-400 text-lg">🔎</span>
                    @if(showSearchHistoryPopup){
                        <div class="absolute overflow-y-auto overflow-x-hidden top-full right-0 cursor-default border-purple-400 border-2 z-50 max-h-150 max-w-fit"
                            (click)="$event.stopPropagation()">
                            <!-- <div class="bg-white shadow-2xl w-full max-w-120 p-4"> -->
                                <table class="border-collapse table-auto ">
                                    <thead class="bg-gray-200">
                                        <tr>
                                            <th class="text-black text-semibold text-left align-top py-1 px-3 pl-7 whitespace-nowrap min-w-150 border-b border-gray-200">
                                                Criteria (Last 15 Results)</th>
                                            <th class="text-black text-semibold text-left align-top py-1 px-3 pr-6 whitespace-nowrap min-w-30">
                                                Searched On</th>
                                        </tr>
                                    </thead>
